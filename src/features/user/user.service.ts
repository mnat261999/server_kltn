...
export class UserService {
	...
	async login(body: LoginDto): Promise<any> {
		try {
			...
			if (!user) throw new HttpException('This email does not exist.', HttpStatus.BAD_REQUEST);

			const isMatch = await bcrypt.compare(body.password, user.password)
			if (!isMatch) throw new HttpException('Password is incorrect', HttpStatus.BAD_REQUEST);


			const refresh_token = this.jwtService.sign({ user }, { expiresIn: '7d', secret: process.env.JWT_SECRECT })

			await this.userModel.findOneAndUpdate({ _id: user._id }, {
				refresh_token: refresh_token
			})

			const userLogin = await this.userModel.findOne({ email: body.email }).select("email")
			return {
	...
	async getAccessToken(body: RefreshTokenDto): Promise<any> {
		try {
			const user = await this.userModel.findOne({ email: body.email })

			const rf_token = user.refresh_token

			if (!rf_token) throw new HttpException('Please login now!', HttpStatus.BAD_REQUEST)

			await this.jwtService.verify(rf_token, <any>process.env.JWT_SECRECT)

			return {
				access_token: rf_token
			}
		} catch (error) {
			console.log('err')
